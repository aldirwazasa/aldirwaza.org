...
function scrollGallery(direction) {
  ...
  strip.scrollBy({ left: (isRtl ? -direction : direction) * itemWidth, behavior: 'smooth' });
}
...
function renderAll() {
  ...
  // Booking dropdown — rebuild options only
...
function prefillBooking(activityId) {
  ...
  // Pre-select activity in dropdown
  const sel = document.getElementById('bf-activity');
  ...
  if (timeEl) timeEl.innerHTML = `<option value="">${t('اختر التاريخ أولاً','Choose a date first')}</option>`;
 
  // Price note from first date
  const priceNote = document.getElementById('bf-price-note');
  if (priceNote) {
    const price = exp.dates[0].price || exp.price || '';
    if (price) {
      priceNote.textContent = t('السعر: ','Price: ') + price;
  ...
  // Note from first date
  const dateNote = document.getElementById('bf-date-note');
  if (dateNote) {
    const note = t(
      exp.dates[0].note || exp.note || '',
      exp.dates[0].noteEn || exp.noteEn || exp.dates[0].note || exp.note || ''
    );
    if (note) { dateNote.textContent = note; dateNote.style.display = 'block'; }
    else        { dateNote.style.display = 'none'; }
  }
  ...
  const receiptGroup = document.querySelector('.receipt-group');
  if (receiptGroup) receiptGroup.style.display = exp.requiresReceipt !== false ? 'block' : 'none';
}
...
function updateSlotsForDate(activityId) {
  ...
  timeEl.innerHTML = `<option value="">${t('اختر الوقت','Choose a time')}</option>` +
  ...
  if (banner && dateObj.spotsLeft !== undefined) {
    ...
    if (spots === 0) {
      ...
      banner.style.background = '#fdf2f2'; banner.style.color = '#8b1a1a';
    } else if (spots <= 3) {
      banner.textContent = t(`تبقّى ${spots} مقاعد فقط!`,`Only ${spots} spots left!`);
      banner.style.background = '#fff8e6'; banner.style.color = '#7a5000';
    } else {
      banner.textContent = t(`${spots} مقعد متاح`,`${spots} spots available`);
      banner.style.background = '#f0f7f0'; banner.style.color = '#2d5a2d';
    }
    ...
    if (gsEl && gsGroup) {
      gsGroup.style.display = spots === 0 ? 'none' : 'block';
      if (spots > 0) {
        gsEl.innerHTML = `<option value="">${t('اختر عدد الأشخاص','Select number of people')}</option>` +
...
async function submitBooking() {
  ...
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    errEl.textContent = t('يرجى إدخال بريد إلكتروني صحيح','Please enter a valid email address');
    errEl.style.display = 'block'; return;
  }
  ...
  const receipt = document.getElementById('bf-receipt') ? document.getElementById('bf-receipt').files[0] : null;
...
function resetBooking() {
  ...
  ['bf-name','bf-email','bf-phone','bf-notes','bf-allergy-text','bf-age'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.value='';
  });
  const actEl = document.getElementById('bf-activity');
  if(actEl) actEl.selectedIndex = 0;
  const dateEl = document.getElementById('bf-date');
  if(dateEl) { dateEl.innerHTML=`<option value="">—</option>`; dateEl.disabled=true; }
  const timeEl = document.getElementById('bf-time');
  if(timeEl) timeEl.innerHTML=`<option value="">—</option>`;
  currentActivityId = null;
  const noRadio = document.querySelector('input[name="allergy"][value="no"]');
  if(noRadio) noRadio.checked = true;
  const ad = document.getElementById('bf-allergy-detail');
  if(ad) ad.style.display='none';
  document.querySelectorAll('.bf-source').forEach(cb=>cb.checked=false);
  const rf = document.getElementById('bf-receipt');
  if(rf) rf.value='';
  ['bf-groupsize-group','bf-unavailable-banner','bf-spots-banner','bf-price-note','bf-date-note'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.style.display='none';
  });
  const ag = document.querySelector('.age-group');     if(ag) ag.style.display='none';
  const alg= document.querySelector('.allergy-group'); if(alg) alg.style.display='none';
  const btn=document.getElementById('booking-submit-btn');
...
async function submitContact() {
  ...
  try {
    const fd=new FormData();
    fd.append('form-name','contact'); fd.append('name',name); fd.append('email',email);
    fd.append('message',msg); fd.append('language',currentLang);
...
function countWords(el) {
  ...
  if(counter){counter.textContent=words+' / 100';counter.style.color=words>100?'#b03030':'var(--stone)';}
  if(words>100){el.value=el.value.trim().split(/\s+/).slice(0,100).join(' ');if(counter)counter.textContent='100 / 100';}
}
...
async function submitJoin() {
  ...
  try {
    const fd=new FormData();
    fd.append('form-name','join'); fd.append('firstName',firstName); fd.append('lastName',lastName);
    fd.append('age',age); fd.append('phone',phone); fd.append('team',team);
    fd.append('why',why); fd.append('language',currentLang);
...
function resetJoin() {
  ...
  ['jf-firstname','jf-lastname','jf-age','jf-phone','jf-why'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  const sel=document.getElementById('jf-team'); if(sel) sel.selectedIndex=0;
  const counter=document.getElementById('jf-wordcount');
  if(counter){counter.textContent='0 / 100';counter.style.color='var(--stone)';}
  const btn=document.getElementById('join-submit-btn');
  if(btn){btn.disabled=false;btn.querySelector('.ar').textContent='إرسال الطلب';btn.querySelector('.en').textContent='Submit Application';}
...
function openArticle(idx) {
  ...
  if(w.image && w.image.trim()){img.src=w.image;img.alt=title;hero.style.display='block';}
  else{hero.style.display='none';}
  document.getElementById('modal-date').textContent  = w.date||'';
  document.getElementById('modal-title').textContent = title;
  document.getElementById('modal-body').innerHTML    = body.split(/\n+/).filter(p=>p.trim()).map(p=>`<p>${p}</p>`).join('');
